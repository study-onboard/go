...
func cleanMapItems() {
	...
	fmt.Println("Before clean items")
	fmt.Println("-----------------------------------------------------")
	for position, staff := range positions {
		fmt.Printf("%s: %s\n", position, staff)
	}
	...
	*positionsPtr = make(map[string]string)
	fmt.Println("After clean items")
	fmt.Printf("Positions length: %d\n", len(positions))
	fmt.Println("-----------------------------------------------------")
	if len(positions) == 0 {
		fmt.Println("Positions empty!")
	} else {
		for position, staff := range positions {
			fmt.Printf("%s: %s\n", position, staff)
		}
